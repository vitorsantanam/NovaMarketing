...
export const strapiClient = {
  ...
  getBlogPost: (slug: string, locale: Locale = 'es'): Promise<StrapiResponse<BlogPost[]>> =>
    strapiRequest(`/api/articles?filters[slug][$eq]=${slug}&populate[page_blocks][populate]=*&locale=${STRAPI_LOCALE[locale]}`),
  ...
  getCaseStudy: (slug: string, locale: Locale = 'es'): Promise<StrapiResponse<CaseStudy[]>> =>
    strapiRequest(`/api/case-studies?filters[slug][$eq]=${slug}&populate[page_blocks][populate]=*&locale=${STRAPI_LOCALE[locale]}`),
