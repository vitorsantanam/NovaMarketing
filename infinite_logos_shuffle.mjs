...
for (const file of files) {
    ...
    // Check if dynamic slider already exist (it should contain 'logos-marquee')
    ...
    if (content.includes('<!-- ═══ LOGOS CLIENTES (SOCIAL PROOF)')) {
        // Tighter Regex matching everything until Section 1 title
        content = content.replace(
            /<!-- ═══ LOGOS CLIENTES \(SOCIAL PROOF\) -->[\s\S]*?<!-- ═══ SECCIÓN 1: AUDITORÍA Y ESTRATEGIA/,
            `${infiniteLogosHtml}\n\n    <!-- ═══ SECCIÓN 1: AUDITORÍA Y ESTRATEGIA`
        );
        fs.writeFileSync(file, content);
        console.log(`Updated logos to dynamic in: ${file.split('\\\\').pop()}`);
    } else {
        console.log(`Logos container missing in: ${file.split('\\\\').pop()}`);
    }
}
console.log('Finished updating logos to Infinite Marquee.');
